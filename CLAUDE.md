...
- **`charts/elasticsearch-query-exporter-queries`** — renders scrape config only: a `ServiceMonitor` (`templates/servicemonitor.yaml`, guarded by `serviceMonitor.enabled`) and/or a VictoriaMetrics `VMServiceScrape` (`templates/vmservicescrape.yaml`, guarded by `vmServiceScrape.enabled`), both **disabled by default** and both built from the *same* `queries` list — one endpoint per entry, `params` built from that query's `indexPattern`/`searchString`/`labelFieldMap`/etc. Deliberately a separate chart/release from the exporter: since there's no static `/metrics` path, every query is its own scrape config, and that set changes independently of (and more often than) the exporter's image/resources/connection settings — splitting them out means a query change isn't a Deployment re-release. It doesn't own a Service; `serviceSelectorLabels` in its values must match the exporter chart's Service labels (defaults assume a release named `elasticsearch-query-exporter`). Its VMServiceScrape per-query relabeling hook is named `metricRelabelConfigs`, not `metricRelabelings` (VM operator's field naming diverges from prometheus-operator's there even though almost everything else matches). `examples/` in each chart has values-driven and hand-written standalone manifests.
