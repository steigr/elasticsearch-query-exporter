...
- `templates/servicemonitor.yaml` renders one `ServiceMonitor` with one endpoint per entry in `serviceMonitor.queries`, each endpoint's `params` built from that query's `index-pattern`/`search-string`/`label_field_map`/etc. — this is the chart's answer to "there's no static `/metrics` path to scrape, each query needs its own scrape config." `templates/vmservicescrape.yaml` renders the VictoriaMetrics-operator equivalent (`VMServiceScrape`, guarded by `vmServiceScrape.enabled`) from that *same* `serviceMonitor.queries` list rather than a separate one — don't add a parallel query list for it; extend the shared one and both templates pick it up. Its per-query relabeling hook is named `metricRelabelConfigs`, not `metricRelabelings` (VM operator's field naming diverges from prometheus-operator's there even though almost everything else matches). `charts/elasticsearch-query-exporter/examples/` has a values-driven example (`values-eck.yaml`) and hand-written standalone manifests for both CRDs.
